...
if( ( $_SESSION['user'] != null || $_SESSION['pass'] != null ) && ($member->checklogin($_SESSION['user'],$_SESSION['pass'])) ){
				echo '<div class="ui menu">
							...
							<a href="email" class="active item">
				...
				echo '<div class="ui stacked segments">
							  <div class="ui segment">
								<h2 class="ui header">
								  ...
								  <div class="content">
									Change Email
									<div class="sub header">เปลี่ยนอีเมล์ผู้ใช้งาน User ID : <span style="color:red;text-transform: uppercase;">',$_SESSION['user'],'</span></div>
								  </div>
								</h2>
							  </div>';
if($_POST['submit_email']){
		$ok = $member->changeemail($member->aidonweb($_SESSION['user']),$_POST['mailold'],$_POST['mailnew1'],$_POST['passold']);
			if($ok){
				echo ''.$ok.'';
			}
	}							  
							 echo '<form class="ui form segment" method="post">
								  ...
								  <div class="two fields">
									<div class="field">
									  ...
									  <input type="email" name="mailold" placeholder="อีเมล์ปัจจุบัน">
									</div>
									<div class="field">
									  <label>Current Email Confirm</label>
									  <input type="email" name="mailnew1" placeholder="อีเมล์ใหม่ (ex.yourname@gmail.com)">
									</div>
								  </div>
								  <div class="field">
								  ...
								  <button value="submit_email" name="submit_email" class="ui green button" type="submit">Save</button>
								</form>
							  <div class="ui segment">
